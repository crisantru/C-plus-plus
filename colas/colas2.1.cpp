...
ColaInt* ColaInt::unirColasOrdenadas(ColaInt* cola1, ColaInt* cola2)
{
    ...
    if(cola1->colaVacia() || cola2->colaVacia())
    {
        cout << "/nUna de las colas estan vacias";
        if(cola1 == NULL)
            return cola1;
        else
            return cola2;
    ...
    ColaInt* colaTemporal = new ColaInt();
    NodoInt* aux = cola1->frente;
    int valor = cola1->frente->getValor();
    ...
    for(int i = 0; i < 2; i++)
    {
        while(aux != NULL)
        {
            if(colaTemporal->colaVacia())
            {
                    colaTemporal->encolar(aux->getValor());
